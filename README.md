...
4. Compress the lightweight weights of the trained model into the deberta_lora_weights.zip file. Compress the previously downloaded deberta-v3-base-local model into a zip file as well, and upload both to Kaggle as datasets.
    <br>4-1. Download and use the deberta-lora-weights.zip file from the arcroot branch.
6. Use these as input datasets in the Kaggle notebook environment.
7. Enable the GPU T4 x2 accelerator in the Kaggle notebook to proceed with training.
